package com.group.rua.Session.Unit.sign_up;
...
class SignupServiceTest {
    ...
    void verifyTokenTest_tokenExists() {
        //Arrange
        //usamos el usuario prefabricado que ya tiene correo
        UnconfirmedUser user = createUnconfirmedUser();
    ...
    @Test
    void createUser_EmailFails_ShouldCatchException() {
        UnconfirmedUser user = createUnconfirmedUser();

        when(passwordEncoder.encode(anyString())).thenReturn("hashedPassword");
        when(unconfirmedUserRepo.save(any(UnconfirmedUser.class))).thenReturn(user);
        // Hacemos que el envío de correo explote a propósito
        when(mailSender.createMimeMessage()).thenThrow(new RuntimeException("Fallo simulado de correo"));
        assertDoesNotThrow(() -> signupService.createUser(user));
    }
