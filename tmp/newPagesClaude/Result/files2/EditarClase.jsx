...
function EditarClase() {
  ...
  // Estado guardado (simula lo que vendría del backend) — TODO: GET /class/:classId
  const [savedCls] = useState({
    id: classId,
    courseId: "INF-301",
  ...
  // Estado de trabajo: se modifica libremente hasta guardar
  const [pendingCls, setPendingCls] = useState(savedCls);

  // Tipo de cambio pendiente para mostrar en la barra
  // Valores posibles: null | "alumno" | "anulacion" | "eliminacion"
  ...
  // ── Guardar: aplica el cambio pendiente al backend ────────────────────────
  const handleGuardar = () => {
    if (pendingAction === "eliminacion") {
      // TODO: DELETE /class/:classId
      alert("Clase eliminada (mock). Redirigiendo al panel.");
      navigate("/docente");
      return;
    }
    if (pendingAction === "anulacion") {
      // TODO: PATCH /class/:classId  →  { isCancelled: pendingCls.isCancelled }
    }
    if (pendingAction === "alumno") {
      // TODO: POST /class/:classId/students  →  { email } por cada alumno nuevo
    }
    alert("Cambios guardados (mock). Aquí irán las llamadas al backend.");
    setPendingAction(null);
  };

  // ── Cancelar: descarta el cambio y restaura el estado guardado ────────────
  const handleCancelar = () => {
    setPendingCls(savedCls);
    setPendingAction(null);
  };

  // Texto descriptivo de la barra según el tipo de cambio pendiente
  ...
  // Ítems del menú de opciones — el orden puede ajustarse según lógica de negocio
  const menuItems = [
    {
      emoji: "📋",
    ...
    },
    {
      ...
      action: () => navigate(`/docente/asistencia/${pendingCls.id}`),
      variante: "normal",
    },
    {
      emoji: "✕",
    ...
    <div className="pagina">
      ...
      <main className="derecha">
        ...
        <Card>
          ...
          {/* Badge de clase anulada (refleja el estado pendiente) */}
          {pendingCls.isCancelled && (
            <span className="badge-anulada">CLASE ANULADA</span>
          )}

          {/* Barra de cambios pendientes — aparece ante cualquier cambio no guardado */}
          {hasPendingChanges && (
